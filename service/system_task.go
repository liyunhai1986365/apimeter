...
func StartSystemTaskRunner() {
	systemTaskRunnerOnce.Do(func() {
		...
		// Start a background goroutine to update active instances count
		gopool.Go(func() {
			ticker := time.NewTicker(30 * time.Second)
			defer ticker.Stop()
			for {
				count, err := model.CountActiveSystemInstances(common.GetTimestamp())
				if err == nil {
...
func runSystemTaskClaimPass(runnerID string) {
	...
	for _, handler := range handlers {
		...
		if !claimed {
			// Record lock wait time even if not claimed (lock contention)
			RecordSystemTaskLockWait(handler.Type(), time.Since(lockStartTime).Seconds())
			continue
		}
		// Record successful lock acquisition time
		RecordSystemTaskLockWait(handler.Type(), time.Since(lockStartTime).Seconds())

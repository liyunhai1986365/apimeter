/*
Copyright (C) 2023-2026 QuantumNous

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU Affero General Public License as published by
the Free Software Foundation, either version 3 of the License, or
(at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU Affero General Public License for more details.

You should have received a copy of the GNU Affero General Public License
along with this program. If not, see <https://www.gnu.org/licenses/>.

For commercial licensing, please contact support@quantumnous.com
*/
import type { AnnouncementType } from '@/features/dashboard/lib/announcement-categories'
import type { AnnouncementAudience } from '../types'

export type Announcement = {
  id: number
  title: string
  content: string
  publishDate: string
  type: AnnouncementType
  extra?: string
  audience: AnnouncementAudience
}

export type AnnouncementDraft = Omit<Announcement, 'id'>

type PublicationPlanInput = {
  announcements: Announcement[]
  draft: AnnouncementDraft
  editingId?: number
  displayOnFrontend: boolean
}

type PublicationPlan = {
  announcements: Announcement[]
  shouldSave: boolean
}

export function planAnnouncementPublication({
  announcements,
  draft,
  editingId,
  displayOnFrontend,
}: PublicationPlanInput): PublicationPlan {
  if (!displayOnFrontend) {
    if (editingId === undefined) {
      return { announcements, shouldSave: false }
    }

    return {
      announcements: announcements.filter((item) => item.id !== editingId),
      shouldSave: true,
    }
  }

  if (editingId !== undefined) {
    return {
      announcements: announcements.map((item) =>
        item.id === editingId ? { ...item, ...draft } : item
      ),
      shouldSave: true,
    }
  }

  const newId = Math.max(...announcements.map((item) => item.id), 0) + 1
  return {
    announcements: [...announcements, { id: newId, ...draft }],
    shouldSave: true,
  }
}

...
const URL_TO_CONFIG_MAP: Record<string, { section: string; module: string }> = {
  ...
  '/dashboard/billing': { section: 'console', module: 'detail' },
  ...
  // '/system-tasks': { section: 'admin', module: 'system_task' }, // Commented out to use default visibility
}
...
function isModuleEnabled(
  ...
  agentConsoleAllowed: boolean
): boolean {
  if (url === '/agents' && !agentConsoleAllowed) {
    return false
  }
...
export function useSidebarConfig(navGroups: NavGroup[]): NavGroup[] {
  ...
  const agentConsoleAllowed = canUseAgentConsole(
    auth?.user?.role,
    auth?.user?.has_agent,

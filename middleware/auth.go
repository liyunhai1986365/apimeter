...
func authHelper(c *gin.Context, minRole int) {
	...
	if !enforceWorkspaceAccountPolicy(c, user.ParentUserId) {
		return
	}
...
// BrowserSessionAuth authenticates top-level browser navigations using the
// signed dashboard session and its revocable server-side session. It
// intentionally does not accept management tokens.
func BrowserSessionAuth() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		sessionID, sessionIDOK := session.Get("session_id").(string)
		authVersion, authVersionOK := session.Get("auth_version").(int64)
		sessionVersion, sessionVersionOK := session.Get("session_version").(int64)
		if !idOK || id <= 0 || !usernameOK || !roleOK || !statusOK || !sessionIDOK || strings.TrimSpace(sessionID) == "" || !authVersionOK || !sessionVersionOK || status != common.UserStatusEnabled || !validUserInfo(username, role) {
		...
		_, userCache, err := service.ValidateLoginSession(identity)
		if err != nil {
			writeDashboardAuthError(c, err)
			return
		}
		setDashboardAuthContext(c, userCache, identity, false)
		c.Set("status", status)
...
func classifyDashboardCredential(c *gin.Context) (*model.UserBase, service.AuthIdentity, dashboardCredentialKind, error) {
	...
	if !ok {
		// The local dashboard still uses its signed browser cookie. The cookie
		// contains only a reference to the new server-side session, so revocation,
		// auth-version changes, expiry and account status remain enforceable.
		if _, hasSessionMiddleware := c.Get(sessions.DefaultKey); hasSessionMiddleware {
			legacySession := sessions.Default(c)
			userID, idOK := legacySession.Get("id").(int)
			sid, sidOK := legacySession.Get("session_id").(string)
			userAuthVersion, authVersionOK := legacySession.Get("auth_version").(int64)
			sessionVersion, sessionVersionOK := legacySession.Get("session_version").(int64)
			if idOK && sidOK && authVersionOK && sessionVersionOK && userID > 0 && strings.TrimSpace(sid) != "" {
				identity := service.AuthIdentity{
					UserID: userID, SessionID: sid,
					UserAuthVersion: userAuthVersion, SessionVersion: sessionVersion,
				}
...
func TokenAuth() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if !RequireTokenAgentAccess(c, token.UserId) {
			return
		}

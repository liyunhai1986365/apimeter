...
type imageAsyncTaskStateResponse struct {
	ID         string                 `json:"id"`
	State      string                 `json:"state"`
	Progress   int                    `json:"progress"`
	CreateTime int64                  `json:"create_time"`
...
func newLocalImageAsyncResponseRecorder() *localImageAsyncResponseRecorder {
	return &localImageAsyncResponseRecorder{
		header: make(http.Header),
		code:   http.StatusOK,
	}
}

func (r *localImageAsyncResponseRecorder) Header() http.Header {
	return r.header
}
...
func (r *localImageAsyncResponseRecorder) WriteHeader(statusCode int) {
	r.code = statusCode
}
...
func isImageAsyncSubmitBody(body []byte) bool {
	...
	if err := common.Unmarshal(body, &imageResponse); err == nil && len(imageResponse.Data) > 0 {
		return false
	}
	return true
}
...
func createLocalImageAsyncTask(c *gin.Context, info *relaycommon.RelayInfo, requestBody []byte) (*model.Task, []byte, *types.NewAPIError) {
	...
	body, err := imageTaskSubmittedResponse(task)
	if err != nil {
...
func startLocalImageAsyncTask(c *gin.Context, info *relaycommon.RelayInfo, task *model.Task, requestBody []byte) {
	...
	asyncInfo.Billing = nil
	asyncInfo.ForcePreConsume = false
	if strings.TrimSpace(asyncInfo.RequestURLPath) == "" {
		asyncInfo.RequestURLPath = "/v1/images/generations"
	}
	go runLocalImageAsyncTask(cloneImageAsyncGinContext(c, requestBody), &asyncInfo, task)
}
...
func cloneImageAsyncGinContext(c *gin.Context, requestBody []byte) *gin.Context {
	...
	if err != nil {
		request, _ = http.NewRequest(http.MethodPost, "/v1/images/generations", bytes.NewReader(requestBody))
	}
	...
	if c != nil {
		for _, key := range []string{
			...
			string(constant.ContextKeyChannelRetryEnabled),
			...
			string(constant.ContextKeyTokenBillingSource),
			string(constant.ContextKeyTokenSubscriptionPlanId),
			string(constant.ContextKeyTokenUserSubscriptionId),
			string(constant.ContextKeyOriginalModel),
			common.RequestIdKey,
			common.KeyRequestBody,
		} {
			if value, ok := c.Get(key); ok {
				asyncCtx.Set(key, value)
			}
		}
		if settings, ok := c.Get(string(constant.ContextKeyUserSetting)); ok {
			asyncCtx.Set(string(constant.ContextKeyUserSetting), settings)
		}
	}
...
func runLocalImageAsyncTask(c *gin.Context, info *relaycommon.RelayInfo, task *model.Task) {
	...
	reloaded, exist, err := model.GetByOnlyTaskId(task.TaskID)
	if err == nil && exist && reloaded != nil {
		task = reloaded
	}
	recorder, _ := c.Get(localImageAsyncRecorderKey)
	responseRecorder, _ := recorder.(*localImageAsyncResponseRecorder)
	if apiErr == nil && responseRecorder != nil && isImageAsyncSubmitBody(responseRecorder.body.Bytes()) {
		return
	}
	snap = task.Snapshot()
	now := time.Now()
	...
	if apiErr != nil {
		...
		errBody, _ := imageTaskStateResponseBytes(task, dto.ImageTaskData{}, map[string]interface{}{
			"message": apiErr.Error(),
			"code":    string(apiErr.GetErrorCode()),
		})
		task.Data = errBody
		if won, updateErr := task.UpdateWithStatus(snap.Status); updateErr == nil && won && task.Quota != 0 {
			service.RefundTaskQuota(context.Background(), task, task.FailReason)
		}
		return
	}

	if responseRecorder == nil || responseRecorder.body.Len() == 0 {
		task.Status = model.TaskStatusFailure
		task.FailReason = "image response body is empty"
		errBody, _ := imageTaskStateResponseBytes(task, dto.ImageTaskData{}, map[string]interface{}{
	...
	taskBody, err := localImageTaskResponseFromOpenAIImageBody(task.TaskID, task.Action, now, responseRecorder.body.Bytes())
	if err != nil {
		task.Status = model.TaskStatusFailure
	...
	if err := common.Unmarshal(taskBody, &taskResp); err == nil {
		task.PrivateData.ResultURL = firstRelayImageURL(taskResp.Data.Images)
	}
	_, _ = task.UpdateWithStatus(snap.Status)
}
...
func isLocalImageAsyncWorker(c *gin.Context) bool {
	...
	return c.GetBool(localImageAsyncWorkerKey)
}
...
func imageTaskSubmittedResponse(task *model.Task) ([]byte, error) {
	if task == nil {
...
func imageTaskStateResponseBytes(task *model.Task, data dto.ImageTaskData, errData map[string]interface{}) ([]byte, error) {
	...
	if strings.TrimSpace(task.Progress) != "" {
		if value, err := strconv.Atoi(strings.TrimSuffix(strings.TrimSpace(task.Progress), "%")); err == nil {
			progress = value
		}
	}
	response := imageAsyncTaskStateResponse{
		ID:         task.TaskID,
		State:      mapTaskStatusToSimple(task.Status),
		Progress:   progress,
	...
	if response.CreateTime == 0 {
		response.CreateTime = task.SubmitTime
	}
	if response.UpdateTime == 0 {
		response.UpdateTime = common.GetTimestamp()
	}
	if response.Action == "" {
		response.Action = constant.TaskActionGenerate
	}
...
func localImageTaskResponseFromOpenAIImageBody(taskID, action string, now time.Time, body []byte) ([]byte, error) {
	...
	createTime := imageResponse.Created
	if createTime == 0 {
		createTime = now.Unix()
	}
	...
	return common.Marshal(imageAsyncTaskStateResponse{
		ID:         taskID,
		State:      "succeeded",
		Progress:   100,
...
func convertLocalImageTaskResponse(task *model.Task) ([]byte, error) {
	...
	if len(task.Data) > 0 {
		...
		if err := common.Unmarshal(task.Data, &response); err == nil && response.ID != "" {

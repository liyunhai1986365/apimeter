...
func TestGetConfigurableProtocolProfilesIncludesEmbeddedProfiles(t *testing.T) {
	...
	require.Equal(t, "seedance2-apimeter", gjson.GetBytes(body, `data.#(id=="seedance2-apimeter").id`).String(), string(body))
	require.Equal(t, "Seedance 2.0 APIMeter", gjson.GetBytes(body, `data.#(id=="seedance2-apimeter").name`).String(), string(body))
}

...
func channelHasSensitiveChanges(channel *PatchChannel, origin *model.Channel, requestData map[string]any) bool {
	...
	if _, ok := requestData["channel_ratio"]; ok && !equalFloat64Ptr(channel.ChannelRatio, origin.ChannelRatio) {
		return true
	}
	if _, ok := requestData["scope"]; ok && channel.Scope != origin.Scope {
		return true
	}
	if _, ok := requestData["owner_user_id"]; ok && channel.OwnerUserId != origin.OwnerUserId {
		return true
	}

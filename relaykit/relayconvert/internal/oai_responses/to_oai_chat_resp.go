...
func UsageFromResponsesUsage(src *dto.Usage) *dto.Usage {
	...
	completionDetails := src.CompletionTokenDetails
	if src.OutputTokensDetails != nil {
		completionDetails = *src.OutputTokensDetails
	}
	if completionDetails.ReasoningTokens != 0 ||
		completionDetails.TextTokens != 0 ||
		completionDetails.AudioTokens != 0 ||
		completionDetails.ImageTokens != 0 ||
		completionDetails.AcceptedPredictionTokens != 0 ||
		completionDetails.RejectedPredictionTokens != 0 {
		usage.CompletionTokenDetails = completionDetails
	}
